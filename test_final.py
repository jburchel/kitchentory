...
Total: $11.74
    ...
    for test_case in test_cases:
        ...
        # Collect results
        valid_items = [item for item in parsed_receipt.items if item.price and item.price > 0]
        result = {
        ...
        print(f"   Items Found: {result['items_found']} ({result['valid_items']} valid)")
        ...
        if valid_items:
            ...
            for item in valid_items[:3]:  # Show first 3 items
                ...
                print(f"       Category: {item.category}, Confidence: {item.confidence_score:.1%}")
    ...
    for result in results:
        ...
        print(f"{status} {result['store']}: {result['valid_items']} items, {result['confidence']:.1%} confidence")
    ...
    print(f"   Stores with >80% Confidence: {sum(1 for r in results if r['confidence'] > 0.8)}/{len(results)}")
    
    # Feature verification
    print(f"\n🔍 Feature Verification:")
    print(f"   ✅ Store Detection: {sum(1 for r in results if r['detected_store'] != 'Unknown Store')}/{len(results)} stores")
    print(f"   ✅ Date Extraction: {sum(1 for r in results if r['date'])}/{len(results)} receipts")
    print(f"   ✅ Total Extraction: {sum(1 for r in results if r['total'])}/{len(results)} receipts")
