...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--user-id',
            type=int,
            help='Generate alerts for specific user ID only'
        )
        parser.add_argument(
            '--days-ahead',
            type=int,
        ...
        for user in users:
            try:
                if dry_run:
                    ...
                    alert_counts = generate_expiration_alerts(user, days_ahead=days_ahead)
                    ...
                    for alert_type in ['expired', 'expiring_soon', 'use_first', 'opened_item']:
