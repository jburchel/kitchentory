...
class InventoryItemQuerySet(models.QuerySet):
    ...
    def expired(self):
        ...
        return self.filter(expiration_date__lt=timezone.now().date(), is_consumed=False)
    ...
    def low_stock(self):
        ...
        return self.filter(
            current_quantity__lt=models.F("minimum_threshold"), is_consumed=False
        )
...
class StorageLocation(models.Model):
    ...
    household = models.ForeignKey(
        "accounts.Household", on_delete=models.CASCADE, related_name="storage_locations"
    )
    name = models.CharField(_("name"), max_length=100)
    location_type = models.CharField(
        _("type"), max_length=20, choices=LOCATION_TYPES, default="pantry"
    )
    ...
    temperature_min = models.FloatField(
        _("minimum temperature (°C)"), null=True, blank=True
    )
...
class Product(models.Model):
    ...
    barcode = models.CharField(
        _("barcode"), max_length=50, unique=True, null=True, blank=True, db_index=True
    )
    ...
    local_image = models.ImageField(
        _("local image"), upload_to="products/", blank=True, null=True
    )
    local_thumbnail = models.ImageField(
        _("local thumbnail"), upload_to="products/thumbs/", blank=True, null=True
    )
    ...
    average_price = models.DecimalField(
        _("average price"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    def _fuzzy_search(cls, query):
        """
        PostgreSQL full-text search with fuzzy matching capabilities.
        Uses trigram similarity and full-text search for better performance.
        """
        if not query.strip():
            return cls.objects.none()

        # Create search vector from name, brand, and description
        search_vector = (
        ...
        # If few results, also try trigram similarity for fuzzy matching
        ...
        try:
            ...
            trigram_results = (
                cls.objects.annotate(
                    ...
                    brand_similarity=TrigramSimilarity("brand", query),
                )
                .filter(
                    models.Q(search=search_query)
                    | models.Q(name_similarity__gte=0.3)
                    | models.Q(brand_similarity__gte=0.3)
                )
                .order_by("-rank", "-name_similarity", "-brand_similarity")
            )

            return trigram_results[:10]

        except ImportError:
            # Fallback to basic icontains search if trigram not available
...
class InventoryItem(models.Model):
    ...
    household = models.ForeignKey(
        "accounts.Household", on_delete=models.CASCADE, related_name="inventory_items"
    )
    ...
    unit = models.CharField(
        _("unit"), max_length=10, choices=QUANTITY_UNITS, default="count"
    )
    ...
    consumed_date = models.DateTimeField(_("consumed date"), null=True, blank=True)
    ...
    def is_low_stock(self):
        ...
        # minimum_threshold has default=0, so we check if it's greater than 0
...
class ProductBarcode(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="additional_barcodes"
    )
    ...
    package_size = models.CharField(
        _("package size"), max_length=50, blank=True, help_text=_("e.g., 500ml, 1kg")
    )
