...
from accounts.permissions import household_required
...
from .models import Product, InventoryItem, Category, StorageLocation, ProductBarcode
from .forms import (
    ProductForm,
    InventoryItemForm,
    QuickAddForm,
...
def inventory_dashboard(request):
    ...
    items = InventoryItem.objects.filter(household=household, is_consumed=False)
    ...
    stats = {
        ...
        "expired": items.filter(expiration_date__lt=date.today()).count(),
        "categories": items.values("product__category__name")
    ...
    expiring_items = (
        items.filter(expiration_date__lte=date.today() + timedelta(days=7))
...
def inventory_list(request):
    ...
    items = InventoryItem.objects.filter(
        household=household, is_consumed=False
    ).select_related("product", "product__category", "location")
    ...
    categories = (
        ...
        .distinct()
        .annotate(item_count=Count("products__inventory_items"))
    )

    locations = StorageLocation.objects.filter(household=household).annotate(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.headers.get("HX-Request"):
                return render(
                    request, "inventory/partials/item_card.html", {"item": item}
                )
...
def product_search(request):
    ...
    if format_type == "html":
        ...
        context = {
            ...
            "fuzzy_message": (
                f'Did you mean one of these? (searched for "{query}")'
                if is_fuzzy_search
                else None
            ),
        }
        html = render_to_string(
            "inventory/partials/search_results.html", context, request
        )
            ...
            results.append(
                {
                    ...
                    "image_url": product.image_url or "",
                    "local_image": (
                        product.local_image.url if product.local_image else ""
                    ),
                    "barcode": product.barcode or "",
                }
            )
...
def storage_locations(request):
    ...
    locations = StorageLocation.objects.filter(
        household=request.user.household
    ).annotate(item_count=Count("items", filter=Q(items__is_consumed=False)))
...
def add_product_manual(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            product = form.save()
            messages.success(
                request, _('Product "{}" added successfully!').format(product.name)
            )
