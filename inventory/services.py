...
class ProductDataService:
    ...
    def _parse_open_food_facts_data(
        self, product_info: Dict, barcode: str
    ) -> Dict[str, Any]:
        """
        Parse Open Food Facts product data into standardized format.
        """
        # Extract basic product information
        name = product_info.get("product_name") or product_info.get(
            "product_name_en", ""
        )
        brand = (
            product_info.get("brands", "").split(",")[0].strip()
        ...
        ingredients_text = product_info.get("ingredients_text_en") or product_info.get(
            "ingredients_text", ""
        )
    ...
    def _get_best_image_url(
        self, product_info: Dict, image_type: str = "front", size: str = "400"
    ) -> Optional[str]:
        """
        ...
        if "front" in selected_images:
            ...
            if "display" in front_images:
                return front_images["display"].get("en") or front_images["display"].get(
                    "fr"
                )
        ...
        try:
            ...
            if not created:
                ...
                product.serving_size = (
                    product_data.get("serving_size") or product.serving_size
                )
                ...
                product.description = (
                    product_data.get("description") or product.description
                )
                product.ingredients = (
                    product_data.get("ingredients") or product.ingredients
                )
            ...
            logger.info(
                f"{'Created' if created else 'Updated'} product: {product.name} ({barcode})"
            )
            return product
    ...
    def bulk_enrich_products(self, barcodes: List[str]) -> Dict[str, Any]:
        ...
        """
        results = {"success": [], "failed": [], "total": len(barcodes)}
                ...
                if product_data:
                    ...
                    if product:
                        ...
                        results["failed"].append(
                            {"barcode": barcode, "error": "Failed to create product"}
                        )
                else:
                    results["failed"].append(
                        {"barcode": barcode, "error": "Product not found"}
                    )
            except Exception as e:
                results["failed"].append({"barcode": barcode, "error": str(e)})

        logger.info(
            f"Bulk enrichment complete: {len(results['success'])}/{results['total']} successful"
        )
        return results

    def download_product_image(
        self, image_url: str, product_name: str, image_type: str = "product"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            response = requests.get(
                image_url, timeout=30, headers={"User-Agent": "Kitchentory/1.0"}
            )
    ...
    def process_product_images(self, product_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if product_data.get("image_url"):
            local_path = self.download_product_image(
                product_data["image_url"], product_data.get("name", "product"), "main"
            )
            if local_path:
        ...
        if product_data.get("thumbnail_url") and product_data.get(
            "thumbnail_url"
    ...
    def create_or_update_product_with_images(
        self, product_data: Dict[str, Any]
    ) -> Optional[Product]:
        """
