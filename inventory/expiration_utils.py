...
def generate_expiration_alerts(user, days_ahead: int = 7) -> Dict[str, int]:
    ...
    inventory_items = InventoryItem.objects.filter(
        user=user, is_consumed=False, quantity__gt=0, expiration_date__isnull=False
    )
    ...
    for item in inventory_items:
        ...
        existing_alert = ExpirationAlert.objects.filter(
            user=user, inventory_item=item, alert_date=today, is_dismissed=False
        ).exists()
        ...
        if item.opened_date and (
            today - item.opened_date
        ).days >= _get_opened_shelf_life(item):
...
def _get_opened_shelf_life(item) -> int:
    ...
    rules = (
        ExpirationRule.objects.filter(user=item.user, is_active=True)
        .filter(Q(product=item.product) | Q(category=item.product.category))
        .order_by("product", "category")
    )  # Product rules take precedence
...
def log_waste(
    ...
    if inventory_item:
        ...
        category_name = (
            ...
            if inventory_item.product.category
            else ""
        )
    ...
    if not estimated_value and inventory_item:
        if inventory_item.price and inventory_item.quantity > 0:
            ...
            estimated_value = (
                inventory_item.price * quantity_wasted
...
def get_expiration_dashboard_data(user) -> Dict:
    ...
    inventory_items = InventoryItem.objects.filter(
        user=user, is_consumed=False, quantity__gt=0
    )

    total_items = inventory_items.count()
    items_with_expiration = inventory_items.filter(
        expiration_date__isnull=False
    ).count()
    expired_items = inventory_items.filter(expiration_date__lt=today).count()
    expiring_soon = inventory_items.filter(
        expiration_date__gte=today, expiration_date__lte=today + timedelta(days=3)
    ).count()

    # Recent waste statistics (last 30 days)
    thirty_days_ago = today - timedelta(days=30)
    recent_waste = WasteLog.objects.filter(user=user, waste_date__gte=thirty_days_ago)

    waste_stats = {
        "total_items_wasted": recent_waste.count(),
        "total_value_wasted": recent_waste.aggregate(total=Sum("estimated_value"))[
            "total"
        ]
        or Decimal("0"),
        "preventable_waste": recent_waste.filter(
    ...
    # Waste by reason
    waste_by_reason = (
        recent_waste.values("waste_reason")
    ...
    return {
        ...
        "inventory": {
            ...
            "tracking_percentage": (
                (items_with_expiration / total_items * 100) if total_items > 0 else 0
            ),
        },
        "waste": waste_stats,
...
def get_waste_analytics(user, days: int = 30) -> Dict:
    ...
    waste_logs = WasteLog.objects.filter(user=user, waste_date__gte=cutoff_date)

    total_waste = waste_logs.count()
    total_value = waste_logs.aggregate(Sum("estimated_value"))[
    ...
    # Waste by category
    waste_by_category = (
        waste_logs.values("category_name")
        .annotate(count=Count("id"), total_value=Sum("estimated_value"))
        .order_by("-count")
    )
    ...
    # Waste by reason
    waste_by_reason = (
        waste_logs.values("waste_reason")
        .annotate(count=Count("id"), total_value=Sum("estimated_value"))
        .order_by("-count")
    )
    ...
    for week in range(4):  # Last 4 weeks
        ...
        week_waste = waste_logs.filter(
            waste_date__gte=week_start, waste_date__lt=week_end
        )
        ...
        waste_trends.append(
            {
                ...
                "total_value": week_waste.aggregate(Sum("estimated_value"))[
                    "estimated_value__sum"
                ]
                or Decimal("0"),
            }
    ...
    return {
        ...
        "preventable_percentage": (
            (preventable_waste / total_waste * 100) if total_waste > 0 else 0
        ),
        ...
        "estimated_annual_waste_value": (
            float(total_value) * (365 / days) if total_value > 0 else 0
        ),
    }
