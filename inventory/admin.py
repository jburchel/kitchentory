...
from .models import (
    Category, StorageLocation, Product, InventoryItem,
    ProductBarcode
)
...
class ProductAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'brand', 'category', 'barcode')
        }),
        (_('Nutritional Information'), {
            'fields': ('serving_size', 'calories'),
            'classes': ('collapse',)
        }),
        (_('Images'), {
        ...
        (_('Metadata'), {
            'fields': ('source', 'verified', 'created_at', 'updated_at')
        })
    )
...
class InventoryItemAdmin(admin.ModelAdmin):
    list_display = (
        'product_name', 'user', 'quantity_display', 'location',
        'expiration_status', 'is_consumed'
    )
    list_filter = (
        'is_consumed', ExpiringItemFilter, 'location',
        'location__location_type', 'product__category'
    )
    search_fields = (
        'product__name', 'product__brand', 'user__email',
    ...
    fieldsets = (
        (None, {
            'fields': ('id', 'user', 'household', 'product')
        }),
        (_('Quantity & Location'), {
