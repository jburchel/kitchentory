...
Greek Yogurt,Organic Valley,4,cup,6.98,Dairy,Fridge,2024-02-05,Vanilla flavor"""
    ...
    try:
        ...
        if 'sample_items' in preview and preview['sample_items']:
            ...
            for i, item in enumerate(preview['sample_items'][:3], 1):
                print(f"     {i}. {item.get('name', 'Unknown')} - {item.get('quantity', 1)} {item.get('unit', 'item')}")
        ...
        if preview['invalid_rows'] > 0 and 'errors' in preview:
            ...
            for error in preview['errors'][:2]:  # Show first 2 errors
                print(f"     - Row {error.get('row', '?')}: {error.get('message', 'Unknown error')}")
    ...
    try:
        ...
        print(f"✅ Excel validation logic: {'WORKING' if not is_valid else 'NEEDS CHECK'}")
...
def test_email_receipts_summary():
    ...
    test_stores = [
        ('Instacart', 'receipts@instacart.com', '2 x Bananas $3.99\n1 x Milk $4.49\nTotal: $8.48'),
        ('Amazon Fresh', 'auto-confirm@amazon.com', 'Chicken Breast, 1 lb $6.99\nSpinach, 5 oz $2.99\nOrder Total: $9.98'),
    ...
    for store_name, sender, body in test_stores:
        ...
        print(f"   ✅ {store_name}: {len(valid_items)} items, {parsed.confidence_score:.1%} confidence")
