...
class Store(models.Model):
    ...
    sections = models.JSONField(
        ...
        default=list,
        help_text=_(
            'List of store sections in order (e.g., ["Produce", "Dairy", "Meat", "Bakery"])'
        ),
    )
    ...
    latitude = models.DecimalField(
        _("latitude"), max_digits=10, decimal_places=8, null=True, blank=True
    )
    longitude = models.DecimalField(
        _("longitude"), max_digits=11, decimal_places=8, null=True, blank=True
    )
...
class ShoppingList(models.Model):
    ...
    shopping_started_at = models.DateTimeField(
        _("shopping started"), null=True, blank=True
    )
    shopping_completed_at = models.DateTimeField(
        _("shopping completed"), null=True, blank=True
    )
    ...
    def is_shopping_active(self):
        ...
        return (
            ...
            and self.shopping_started_at
            and not self.shopping_completed_at
        )
    ...
    def optimize_for_store(self):
        ...
        for item in self.items.all():
            if item.category:
                ...
                item.store_section = self._map_category_to_section(
...
class ShoppingListShare(models.Model):
    ...
    permission = models.CharField(
        _("permission"), max_length=10, choices=PERMISSION_CHOICES, default="view"
    )

    shared_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="shopping_shares_created"
    )
...
class RecurringShoppingItem(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="recurring_shopping_items"
    )
    household = models.ForeignKey(
        "accounts.Household",
        on_delete=models.CASCADE,
    ...
    frequency = models.CharField(
        _("frequency"), max_length=10, choices=FREQUENCY_CHOICES, default="weekly"
    )
