...
class RecipeParser:
    ...
    def _is_recipe_object(self, obj):
        ...
        return (
            "recipe" in obj_type
            or obj.get("recipeIngredient")
            or obj.get("recipeInstructions")
        )
    ...
    def _extract_recipe_from_json_ld(self, recipe_obj):
        ...
        data = {
            ...
            "ingredients": self._get_ingredients(
                recipe_obj.get("recipeIngredient", [])
            ),
            "instructions": self._get_instructions(
                recipe_obj.get("recipeInstructions", [])
            ),
    ...
    def _parse_microdata(self, soup):
        ...
        data = {
            ...
            "prep_time": self._parse_duration(
                self._get_microdata_property(recipe_elem, "prepTime")
            ),
            "cook_time": self._parse_duration(
                self._get_microdata_property(recipe_elem, "cookTime")
            ),
            "total_time": self._parse_duration(
                self._get_microdata_property(recipe_elem, "totalTime")
            ),
...
def create_recipe_from_data(recipe_data, user, make_public=False):
    ...
    for i, instruction_text in enumerate(instructions):
        if instruction_text:
            RecipeStep.objects.create(
                recipe=recipe, step_number=i + 1, instruction=instruction_text[:1000]
            )
    ...
    for keyword in keywords[:5]:  # Limit to 5 tags
        tag, created = RecipeTag.objects.get_or_create(
            name=keyword[:50], defaults={"slug": slugify(keyword)}
        )
        recipe.tags.add(tag)
