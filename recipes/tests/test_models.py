...
class RecipeModelTest(TestCase):
    ...
    def test_recipe_slug_generation(self):
        ...
        recipe = Recipe.objects.create(
            name="Amazing Pasta Recipe!", category=self.category, created_by=self.user
        )
...
class RecipeIngredientModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.recipe = Recipe.objects.create(
            name="Test Recipe", category=self.recipe_category, created_by=self.user
        )
        self.product_category = Category.objects.create(name="Vegetables")
        self.product = Product.objects.create(
            name="Tomato", category=self.product_category
        )
...
class RecipeStepModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.category = RecipeCategory.objects.create(name="Main Course")
        self.recipe = Recipe.objects.create(
            name="Test Recipe", category=self.category, created_by=self.user
        )
    ...
    def test_recipe_step_ordering(self):
        ...
        step2 = RecipeStep.objects.create(
            recipe=self.recipe, step_number=2, instruction="Add ingredients"
        )
...
class RecipeTagModelTest(TestCase):
    ...
    def setUp(self):
        self.tag = RecipeTag.objects.create(
            name="vegetarian", description="Suitable for vegetarians"
        )
...
class UserRecipeInteractionModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.category = RecipeCategory.objects.create(name="Main Course")
        self.recipe = Recipe.objects.create(
            name="Test Recipe", category=self.category, created_by=self.chef
        )
    ...
    def test_user_recipe_interaction_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            UserRecipeInteraction.objects.create(
                user=self.user, recipe=self.recipe, interaction_type="rate", rating=3
            )
    ...
    def test_review_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            RecipeReview.objects.create(
                recipe=self.recipe, user=self.user, rating=3, comment="Different review"
            )
