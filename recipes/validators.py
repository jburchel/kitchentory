    ...
    if isinstance(ingredients_data, list):
        ...
        for i, ingredient in enumerate(ingredients_data):
            ...
            if name:
                ...
                if len(name) > 200:
                    errors.append(
                        _('Ingredient {}: Name must be 200 characters or less.').format(i + 1)
                    )
                
                # Check for valid ingredient format
                if not re.search(r'[a-zA-Z]', name):
                    errors.append(
                        _('Ingredient {}: Must contain at least one letter.').format(i + 1)
                    )
        ...
        for i, step in enumerate(instructions_data):
            ...
            if instruction:
                ...
                if len(instruction) < 10:
                    errors.append(
                        _('Step {}: Instruction must be at least 10 characters long.').format(i + 1)
                    )
                
                if len(instruction) > 1000:
                    errors.append(
                        _('Step {}: Instruction must be 1000 characters or less.').format(i + 1)
                    )
...
def validate_recipe_timing(prep_time, cook_time, total_time=None):
    ...
    if not prep_time and not cook_time:
        errors.append(_('Please provide either preparation time, cooking time, or both.'))
        return errors
    ...
    for field_name, (value, min_val, max_val, display_name) in nutrition_fields.items():
        if value is not None:
            try:
                ...
                if value < min_val:
                    errors.append(
                        _('{} cannot be negative.').format(display_name)
                    )
                elif value > max_val:
                    errors.append(
                        _('{} value seems too high (maximum {}).').format(display_name, max_val)
                    )
            except (ValueError, TypeError):
                errors.append(
                    _('{} must be a valid number.').format(display_name)
                )
...
def validate_complete_recipe(recipe_data):
    ...
    ingredients_errors = validate_recipe_ingredients(
    ...
    # Validate servings
    servings_errors = validate_recipe_servings(
