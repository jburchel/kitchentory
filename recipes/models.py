...
class Recipe(models.Model):
    ...
    name = models.CharField(_('name'), max_length=200, db_index=True, default='Untitled Recipe')
    title = models.CharField(_('title'), max_length=200, db_index=True, default='Untitled Recipe')
    ...
    source_url = models.URLField(
        _('source URL'),
        blank=True,
        help_text=_('Original recipe URL if imported')
    )
...
class RecipeIngredient(models.Model):
    ...
    name = models.CharField(
        _('ingredient name'),
        max_length=200,
        help_text=_('Name as it appears in recipe (e.g., "large onion", "fresh basil")')
    )
...
class RecipeStep(models.Model):
    ...
    instruction = models.TextField(
        _('instruction'),
        help_text=_('Detailed step-by-step instruction')
    )
    ...
    def get_next_step(self):
        ...
        return self.recipe.steps.filter(
            step_number__gt=self.step_number
        ).first()
    
    def get_previous_step(self):
        """Get the previous step in the recipe."""
        return self.recipe.steps.filter(
            step_number__lt=self.step_number
        ).last()
...
class RecipeReview(models.Model):
    ...
    made_modifications = models.BooleanField(
        _('made modifications'),
        default=False
    )
    ...
    difficulty_rating = models.IntegerField(
        ...
        null=True,
        blank=True,
        help_text=_('How difficult was this recipe to make? (1=very easy, 5=very hard)')
    )
    would_make_again = models.BooleanField(
        _('would make again'),
        null=True,
        blank=True
    )
...
class CookingSession(models.Model):
    ...
    notes = models.TextField(
        _('session notes'),
        blank=True,
        help_text=_('Notes about this cooking session')
    )
    modifications_made = models.TextField(
        _('modifications made'),
        blank=True,
        help_text=_('Changes made during cooking')
    )
