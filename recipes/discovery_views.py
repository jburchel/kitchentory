...
from .matching import RecipeMatchingEngine, get_recipe_recommendations, check_recipe_cookability
...
def discovery_dashboard(request):
    ...
    almost_matches = [r for r in recommendations if r.match_type in ['almost', 'partial']]
    ...
    for match in matches:
        results.append({
            'recipe': {
                ...
                'category': match.recipe.category.name if match.recipe.category else None,
            },
            'match': {
    ...
    for ing_match in match.ingredient_matches:
        ingredient_details.append({
            'recipe_ingredient': {
                ...
                'unit': ing_match.recipe_ingredient.get_unit_display() if ing_match.recipe_ingredient.unit else '',
            ...
            'inventory_item': {
                'quantity': float(ing_match.inventory_item.quantity) if ing_match.inventory_item else 0,
                'unit': ing_match.inventory_item.unit if ing_match.inventory_item else '',
            } if ing_match.inventory_item else None
        })
    ...
    for ingredient in match.missing_ingredients:
        ...
        if not existing_item:
            shopping_item = ShoppingListItem.objects.create(
                ...
                quantity=Decimal(str(ingredient.quantity)) if ingredient.quantity else Decimal('1.0'),
            ...
            added_items.append({
                ...
                'unit': ingredient.get_unit_display() if hasattr(ingredient, 'get_unit_display') else ingredient.unit,
            })
