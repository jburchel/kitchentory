...
def start_cooking(request, slug):
    ...
    if request.method == "POST":
        ...
        serving_multiplier = (
            requested_servings / recipe.servings if recipe.servings > 0 else 1.0
        )
    else:
    ...
    cooking_session, created = CookingSession.objects.get_or_create(
        ...
        defaults={"servings_planned": requested_servings, "status": "planning"},
    )
...
def cooking_mode(request, slug):
    ...
    serving_multiplier = (
        cooking_session.servings_planned / recipe.servings
...
def update_cooking_session(request, session_id):
    ...
    cooking_session = get_object_or_404(
        CookingSession, id=session_id, user=request.user
    )
    ...
    if action == "pause":
        ...
        UserRecipeInteraction.objects.get_or_create(
            user=request.user, recipe=cooking_session.recipe, interaction_type="cook"
        )
    ...
    return JsonResponse(
        {
            ...
            "current_step": (
                cooking_session.current_step.step_number
                if cooking_session.current_step
                else None
            ),
...
def toggle_ingredient_check(request):
    ...
    return JsonResponse(
        {"success": True, "ingredient_id": ingredient_id, "checked": checked}
    )
        ...
        if recipe_ingredient.product and recipe_ingredient.quantity:
            try:
                inventory_item = InventoryItem.objects.get(
                    user=request.user, product=recipe_ingredient.product
                )
...
def cooking_history(request):
    ...
    """
    sessions = (
        CookingSession.objects.filter(user=request.user)
...
def adjust_serving_size(request):
    ...
    serving_multiplier = (
        cooking_session.servings_planned / cooking_session.recipe.servings
        if cooking_session.recipe.servings > 0
        else 1.0
    )
