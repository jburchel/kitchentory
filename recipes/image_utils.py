...
def save_recipe_image_from_url(recipe, image_url):
    ...
    try:
        ...
        # Save to recipe
        recipe.image.save(processed_image.name, processed_image, save=True)
...
def get_recipe_image_url(recipe, size="medium"):
    ...
    placeholder_sizes = {"small": "300x200", "medium": "600x400", "large": "1200x800"}
...
def cleanup_old_images(days_old=30):
    ...
    old_recipes = Recipe.objects.filter(created_at__lt=cutoff_date, image__isnull=False)
