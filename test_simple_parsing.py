...
Thank you for your order!
    ...
    valid_items = [
        item for item in amazon_receipt.items if item.price and item.price > 0
    ]
...
Thank you for choosing Walmart!
    ...
    valid_walmart_items = [
        item for item in walmart_receipt.items if item.price and item.price > 0
    ]
...
Thank you for shopping with us!
    ...
    print("=" * 50)
    print(
        f"✅ Instacart: {len(parsed_receipt.items)} items, {parsed_receipt.confidence_score:.1%} confidence"
    )
    print(
        f"✅ Amazon Fresh: {len(valid_items)} valid items, {amazon_receipt.confidence_score:.1%} confidence"
    )
    print(
        f"✅ Walmart: {len(valid_walmart_items)} valid items, {walmart_receipt.confidence_score:.1%} confidence"
    )
    print(
        f"✅ Unknown Store: {len(unknown_receipt.items)} items, {unknown_receipt.confidence_score:.1%} confidence"
    )
    ...
    total_items = (
        len(parsed_receipt.items)
        + len(valid_items)
        + len(valid_walmart_items)
        + len(unknown_receipt.items)
    )
