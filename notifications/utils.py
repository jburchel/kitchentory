...
def send_expiration_alerts(user, alerts: List[ExpirationAlert]) -> bool:
    ...
    if preferences.in_app_notifications:
        in_app_sent = _send_expiration_in_app_notifications(
            user, urgent_alerts + high_alerts
        )
...
def _send_expiration_email(
    user,
    all_alerts: List[ExpirationAlert],
    urgent_alerts: List[ExpirationAlert],
    high_alerts: List[ExpirationAlert],
) -> bool:
    """Send expiration alert email."""
    try:
        template = NotificationTemplate.objects.get(
            template_type="expiration_alert", is_active=True
        )
    ...
    text_content = (
        Template(template.text_template).render(context)
...
def _send_expiration_in_app_notifications(user, alerts: List[ExpirationAlert]) -> bool:
    ...
    for alert in alerts:
        try:
            ...
            logger.error(
                f"Failed to create in-app notification for alert {alert.id}: {str(e)}"
            )
...
def send_daily_digest(user) -> bool:
    ...
    try:
        template = NotificationTemplate.objects.get(
            template_type="daily_digest", is_active=True
        )
    except NotificationTemplate.DoesNotExist:
...
def send_weekly_digest(user) -> bool:
    ...
    try:
        template = NotificationTemplate.objects.get(
            template_type="weekly_digest", is_active=True
        )
    except NotificationTemplate.DoesNotExist:
    ...
    return _send_digest_email(
        user, "weekly_digest", subject, html_content, text_content
    )
...
def _prepare_digest_data(user, digest_type: str) -> Dict:
    ...
    active_shopping_lists = ShoppingList.objects.filter(
        created_by=user, status__in=["active", "shopping"]
    ).count()
...
def _should_send_digest(digest_data: Dict) -> bool:
    ...
    return (
        urgent_alerts > 0
        or usage_events > 0
        or digest_data.get("due_predictions", 0) > 0
    )


def _send_digest_email(
    user, template_type: str, subject: str, html_content: str, text_content: str = None
) -> bool:
...
def get_user_notifications(
    user, limit: int = 20, unread_only: bool = False
    ...
    """
    notifications = InAppNotification.objects.filter(user=user, is_dismissed=False)
