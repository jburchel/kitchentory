...
def main():
    ...
    print("📊 Test Summary:")
    print(
        f"✅ Instacart parsing: {len(instacart_result.items)} items, {instacart_result.confidence_score:.1%} confidence"
    )
    print(
        f"✅ Amazon Fresh parsing: {len(amazon_result.items)} items, {amazon_result.confidence_score:.1%} confidence"
    )
    print(
        f"✅ Walmart parsing: {len(walmart_result.items)} items, {walmart_result.confidence_score:.1%} confidence"
    )

    if webhook_result:
        print(
            f"✅ Webhook processing: {webhook_result.get('items_found', 0)} items processed"
        )
    else:
