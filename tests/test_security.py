...
class CSRFSecurityTest(TestCase):
    ...
    def test_csrf_protection_on_login(self):
        ...
        response = self.client.post(
            "/accounts/login/", {"login": "testuser", "password": "testpass123"}
        )
    ...
    def test_csrf_protection_on_forms(self):
        ...
        response = self.client.post("/inventory/add/", {"name": "Test Item"})
        self.assertEqual(response.status_code, 403)

        # Submit with CSRF token should work
        response = self.client.post(
            "/inventory/add/", {"name": "Test Item", "csrfmiddlewaretoken": csrf_token}
        )
...
class AuthenticationSecurityTest(TestCase):
    ...
    def test_rate_limiting_login_attempts(self):
        ...
        for i in range(10):
            response = self.client.post(
                "/accounts/login/", {"login": "testuser", "password": "wrongpassword"}
            )
...
class AuthorizationSecurityTest(TestCase):
    ...
    def setUp(self):
        ...
        self.location1 = StorageLocation.objects.create(
            name="Pantry 1", household=self.household1
        )
        self.location2 = StorageLocation.objects.create(
            name="Pantry 2", household=self.household2
        )
    ...
    def test_user_cannot_modify_other_household_data(self):
        ...
        response = self.client.post(
            f"/inventory/item/{self.item2.id}/edit/", {"current_quantity": "10"}
        )
        self.assertIn(response.status_code, [403, 404])
    ...
    def test_recipe_ownership_protection(self):
        ...
        response = self.client.post(
            f"/recipes/{recipe.slug}/edit/", {"name": "Modified Recipe"}
        )
        self.assertIn(response.status_code, [403, 404])
    ...
    def test_shopping_list_access_control(self):
        ...
        shopping_list = ShoppingList.objects.create(
            name="User1's List", created_by=self.user1, household=self.household1
        )
...
class InputValidationSecurityTest(TestCase):
    ...
    def test_xss_prevention_in_forms(self):
        ...
        for payload in xss_payloads:
            ...
            response = self.client.post(
                ...
                {"name": payload, "category": category.id, "default_unit": "pieces"},
            )
    ...
    def test_sql_injection_prevention(self):
        ...
        for payload in sql_payloads:
            ...
            response = self.client.get("/inventory/search/", {"q": payload})
    ...
    def test_api_input_validation(self):
        ...
        if response.status_code == 200:
            ...
            if token:
                ...
                # Test invalid JSON
                response = self.client.post(
                    "/api/inventory/", "invalid json", **headers
                )
...
class InformationDisclosureTest(TestCase):
    ...
    def test_debug_info_not_exposed(self):
        ...
        response = self.client.post("/inventory/add/", {"invalid_field": "test"})
