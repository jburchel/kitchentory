...
class RecipeAPITest(APITestCase):
    ...
    def test_recipe_ingredients_endpoint(self):
        ...
        product = Product.objects.create(name="Tomato", category=product_category)

        RecipeIngredient.objects.create(
            recipe=self.recipe, product=product, quantity=Decimal("2.0"), unit="pieces"
        )
    ...
    def test_recipe_matching_endpoint(self):
        ...
        location = StorageLocation.objects.create(name="Pantry", household=household)
        product_category = Category.objects.create(name="Vegetables")
        product = Product.objects.create(name="Tomato", category=product_category)
...
class ShoppingAPITest(APITestCase):
    ...
    def test_generate_list_from_depleted_items(self):
        ...
        category = Category.objects.create(name="Test Category")
        product = Product.objects.create(name="Low Stock Product", category=category)
        ...
        data = {"name": "Restock List", "threshold_days": 7}
        ...
        self.assertTrue(new_list.items.filter(product=product).exists())
...
class ValidationAPITest(APITestCase):
    ...
    def test_invalid_data_returns_400(self):
        ...
        url = reverse("api:inventory:list")
        data = {"current_quantity": "invalid_number", "unit": ""}  # Required field
    ...
    def setUp(self):
        ...
        for i in range(25):  # Create more than default page size
            product = Product.objects.create(
                name=f"Product {i}", category=self.category
            )
            InventoryItem.objects.create(
                product=product,
                household=self.household,
