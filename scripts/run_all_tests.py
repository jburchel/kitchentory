...
class TestRunner:
    ...
    def __init__(self, verbose: bool = False, fast: bool = False):
        ...
        self.test_categories = {
            ...
            'unit': {
                'name': 'Unit Tests',
                'tests': ['inventory_models', 'recipe_models', 'shopping_models', 'utility_functions'],
                'required': True
            },
            'integration': {
    ...
    def run_command(self, command: List[str], description: str, 
                   cwd: Optional[str] = None, timeout: int = 300) -> Tuple[bool, str, str]:
    ...
    def run_linting_tests(self) -> Dict[str, bool]:
        ...
        success, _, stderr = self.run_command(['black', '--check', '.'], 'Black format check')
        ...
        success, _, stderr = self.run_command(['isort', '--check-only', '.'], 'isort check')
    ...
    def run_integration_tests(self) -> Dict[str, bool]:
        ...
        success, stdout, stderr = self.run_command(
            ['python', 'manage.py', 'test', 'tests.test_views_integration', '--verbosity=1'],
            'View integration tests'
        )
        ...
        success, stdout, stderr = self.run_command(
            ['python', 'manage.py', 'test', 'tests.test_api_endpoints', '--verbosity=1'],
            'API endpoint tests'
        )
    ...
    def run_security_tests(self) -> Dict[str, bool]:
        ...
        if not success:
            self.log("Dependency vulnerability check failed or tool not available", 'WARNING')
    ...
    def run_all_tests(self) -> Dict[str, Dict[str, bool]]:
        ...
        if not self.check_prerequisites():
            self.log("Prerequisites check failed. Please install missing tools.", 'ERROR')
            return {}
        ...
        for category_key, category_info in self.test_categories.items():
            ...
            try:
                ...
                all_results[category_key] = {test: False for test in category_info['tests']}
    ...
    def save_results(self, results: Dict[str, Dict[str, bool]], report: str):
        ...
        with open(results_file, 'w') as f:
            json.dump({
                ...
                'results': results,
                'summary': {
                    'total_passed': sum(sum(1 for success in cat.values() if success) 
...
def main():
    ...
    parser.add_argument('--fast', '-f', action='store_true', 
                       help='Fast mode (skip optional tests)')
    ...
    total_passed = sum(sum(1 for success in cat.values() if success) for cat in results.values())
    total_tests = sum(len(cat) for cat in results.values())
