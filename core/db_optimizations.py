...
class DatabaseOptimizer:
    ...
    def optimize_inventory_queries():
        ...
        return InventoryItem.objects.select_related(
            "product", "product__category", "location", "household"
        ).prefetch_related("product__barcodes", "expiration_alerts")
    ...
    def get_user_inventory_stats(user, use_cache=True):
        ...
        stats = InventoryItem.objects.filter(household__members=user).aggregate(
            ...
            expiring_soon=Count(
                "id", filter=Q(expiration_date__lte=now + timezone.timedelta(days=7))
            ),
            expired_items=Count("id", filter=Q(expiration_date__lt=now)),
            low_stock=Count(
                "id", filter=Q(current_quantity__lte=F("minimum_threshold"))
            ),
        )
    ...
    def bulk_update_inventory_quantities(updates: List[Dict]):
        ...
        for update in updates:
            try:
                item = InventoryItem.objects.select_for_update().get(id=update["id"])
        ...
        if items_to_update:
            InventoryItem.objects.bulk_update(
                items_to_update, ["current_quantity", "updated_at"]
            )
    ...
    def cleanup_expired_cache_entries():
        ...
        patterns = ["inventory_stats:*", "popular_recipes:*", "shopping_suggestions:*"]
    ...
    def analyze_slow_queries(threshold_ms: float = 100):
        ...
        for query in connection.queries:
            ...
            if time_ms > threshold_ms:
                slow_queries.append({"sql": query["sql"], "time_ms": time_ms})
...
class OptimizedPaginator:
    ...
    def get_page(self, cursor=None):
        ...
        next_cursor = (
            getattr(items[-1], self.cursor_field) if items and has_next else None
        )
