...
def test_csv_import_functionality():
    ...
    household, created = Household.objects.get_or_create(
        name="CSV Test Household", defaults={"invite_code": "CSVTEST"}
    )
...
Bread,Bakery,1,loaf,2.99,Pantry,Counter"""
    ...
    csv_file = SimpleUploadedFile(
        name="test.csv", content=test_csv_data.encode("utf-8"), content_type="text/csv"
    )
...
def test_browser_extension_endpoints():
    ...
    try:
        ...
        print(
            f"      Expected payload: {'items': [{'name': 'str', 'quantity': 'float', ...}]}"
        )
