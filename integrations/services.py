...
class ImportService:
    ...
    def process_import_job(self, job_id: int) -> dict:
        ...
        try:
            ...
            # Get approved items
            approved_items = job.parsed_items.filter(is_approved=True, is_processed=False)
            ...
            if failed_count == 0:
                ...
                job.status = ImportStatus.FAILED if created_count == 0 else ImportStatus.COMPLETED
        ...
        for index, row in df.iterrows():
            try:
                item_data = {
                    ...
                    'quantity': self._parse_quantity(self._get_value(row, column_mapping.get('quantity'))),
                    'unit': self._get_value(row, column_mapping.get('unit'), 'item'),
                    'price': self._parse_price(self._get_value(row, column_mapping.get('price'))),
                    'store': self._get_value(row, column_mapping.get('store')),
                    'purchase_date': self._parse_date(self._get_value(row, column_mapping.get('date'))),
    ...
    def _get_column_mapping(self, columns: list) -> dict:
        ...
        name_columns = ['name', 'product', 'item', 'product_name', 'item_name', 'description']
    ...
    def _create_inventory_item(self, parsed_item: ParsedReceiptItem, job: ImportJob) -> InventoryItem:
        ...
        try:
            ...
            item_data = {
                ...
                'location_id': self.default_location.id if self.default_location else None,
            ...
            if serializer.is_valid():
                ...
                logger.error(f"Serializer errors for {parsed_item.name}: {serializer.errors}")
                return None
    ...
    def _find_or_create_product(self, name: str, brand: str = '', category_name: str = 'Grocery') -> Product:
