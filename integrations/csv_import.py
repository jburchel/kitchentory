...
class CSVImportService:
    ...
    def preview_import(
        self, file: UploadedFile, mapping: ImportMapping = None
    ) -> ImportPreview:
        ...
        try:
            ...
            for i, row in enumerate(sample_rows):
                try:
                    ...
                    errors.append(
                        ValidationError(
                            row=i + 1, field="general", value="", message=str(e)
                        )
                    )
    ...
    def process_import(self, file: UploadedFile, mapping: ImportMapping) -> ImportJob:
        ...
        try:
            ...
            for i, row in enumerate(rows):
                try:
                    ...
                    if validated_item:
                        ...
                        errors.append({"row": i + 1, "error": "Validation failed"})
                except Exception as e:
            ...
            logger.info(
                f"Import job {import_job.id} completed: {created_count} items created"
            )
    ...
    def _validate_row(
        self, row: List[str], mapping: ImportMapping, headers: List[str], row_num: int
    ) -> Optional[Dict]:
    ...
    def _create_inventory_items(
        self, import_job: ImportJob, valid_items: List[Dict]
    ) -> int:
