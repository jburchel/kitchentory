...
def sample_csv(request):
    ...
    try:
        ...
        response["Content-Disposition"] = (
            'attachment; filename="kitchentory_sample_import.csv"'
        )
...
def preview_import(request):
    ...
    try:
        ...
        import_service = CSVImportService(
            household=request.user.household, user=request.user
        )
        ...
        return Response(
            {"error": f"Preview failed: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def process_import(request):
    ...
    if not mapping_data:
        return Response(
            {"error": "Column mapping required"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        ...
        return Response(
            {"error": f"Import failed: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def import_errors(request, job_id):
    ...
    try:
        ...
        return Response(
            {"job_id": import_job.id, "total_errors": len(errors), "errors": errors},
...
def validate_mapping(request):
    ...
    if not mapping_data:
        return Response(
            {"error": "Mapping data required"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def sendgrid_webhook(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": "Webhook processing failed"}, status=500
        )
...
def upload_email_receipt(request):
    ...
    if not raw_email:
        return Response(
            {"error": "Email content required"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def receipt_imports(request):
    ...
    try:
        ...
        receipt_imports = ImportJob.objects.filter(
            household=request.user.household, source=ImportSource.EMAIL_RECEIPT
        ).order_by("-created_at")[
            :20
        ]  # Last 20 receipt imports
