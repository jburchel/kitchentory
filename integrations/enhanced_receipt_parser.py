...
class EnhancedReceiptParser:
    ...
    def _detect_store(self, sender: str, subject: str, body: str) -> Optional[str]:
        ...
        store_indicators = {
            'instacart': [
                'instacart.com', 'instacart', 'your instacart order'
            ],
            'amazon_fresh': [
                'amazon.com', 'amazonfresh', 'amazon fresh', 'whole foods'
            ],
            'walmart': [
                'walmart.com', 'walmart', 'walmart grocery'
            ],
            'target': [
                'target.com', 'target', 'target.com'
            ],
            'kroger': [
                'kroger.com', 'kroger', 'king soopers', 'fred meyer'
            ],
            'safeway': [
                'safeway.com', 'safeway', 'vons', 'pavilions'
            ],
            'costco': [
                'costco.com', 'costco', 'costco wholesale'
            ],
            'wholefoods': [
                'wholefoods', 'whole foods', 'wholefoodsmarket'
            ]
        }
    ...
    def _post_process_receipt(self, receipt: ParsedReceipt):
        ...
        if receipt.items:
            avg_confidence = sum(item.confidence_score for item in receipt.items) / len(receipt.items)
            structure_bonus = 0.1 if receipt.total else 0.0
            date_bonus = 0.1 if receipt.purchase_date else 0.0
            receipt.confidence_score = min(avg_confidence + structure_bonus + date_bonus, 1.0)
        else:
    ...
    def _infer_category(self, item_name: str) -> str:
        ...
        categories = {
            'Produce': [
                ...
                'onion', 'potato', 'broccoli', 'spinach', 'avocado', 'berry',
            ...
            'Meat & Seafood': [
                ...
                'shrimp', 'meat', 'turkey', 'ham', 'bacon'
            ],
            'Pantry': [
                'bread', 'pasta', 'rice', 'cereal', 'beans', 'sauce',
                'oil', 'vinegar', 'flour', 'sugar', 'salt', 'spice'
            ],
            'Frozen': [
                'frozen', 'ice cream', 'pizza', 'frozen fruit', 'frozen vegetable'
            ],
            'Beverages': [
            ...
            'Snacks': [
                'chips', 'crackers', 'nuts', 'candy', 'chocolate', 'cookies',
                'snack', 'granola'
            ],
            'Health & Beauty': [
                'shampoo', 'soap', 'toothpaste', 'vitamin', 'medicine',
                'lotion', 'deodorant'
            ],
            'Household': [
...
class InstacartParser(BaseStoreParser):
    ...
    def parse(self, email_data: Dict) -> ParsedReceipt:
        ...
        # Extract basic info
        receipt.purchase_date = self._extract_date(body, [
            r'delivered on (.+?)(?:\n|$)',
            r'order placed (.+?)(?:\n|$)',
            r'(\d{1,2}/\d{1,2}/\d{4})'
        ])
    ...
    def _parse_instacart_items(self, text: str) -> List[ParsedItem]:
        ...
        for i, line in enumerate(lines):
            ...
            if re.search(r'(your order|items ordered|grocery items)', line, re.IGNORECASE):
    ...
    def _parse_instacart_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                item = ParsedItem(
                    ...
                    quantity=float(quantity) if quantity and quantity.replace('.', '').isdigit() else 1.0,
...
class AmazonFreshParser(BaseStoreParser):
    ...
    def _should_skip_amazon_line(self, line: str) -> bool:
        ...
        skip_patterns = [
            r'order total', r'shipping', r'tax', r'prime', r'discount',
            r'promotion', r'credit', r'gift card', r'your order',
            r'delivered', r'tracking', r'return'
        ]
    ...
    def _parse_amazon_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
...
class WalmartParser(BaseStoreParser):
    ...
    def _should_skip_walmart_line(self, line: str) -> bool:
        ...
        skip_patterns = [
            r'subtotal', r'tax', r'total', r'savings', r'pickup',
            r'discount', r'coupon', r'walmart', r'order', r'confirmation'
        ]
    ...
    def _parse_walmart_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
...
class TargetParser(BaseStoreParser):
    ...
    def _should_skip_target_line(self, line: str) -> bool:
        ...
        skip_patterns = [
            r'subtotal', r'tax', r'total', r'redcard', r'circle',
            r'discount', r'promotion', r'target', r'order', r'pickup'
        ]
    ...
    def _parse_target_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
...
class KrogerParser(BaseStoreParser):
    ...
    def _should_skip_kroger_line(self, line: str) -> bool:
        ...
        skip_patterns = [
            r'subtotal', r'tax', r'total', r'savings', r'fuel points',
            r'discount', r'coupon', r'kroger', r'king soopers', r'fred meyer'
        ]
    ...
    def _parse_kroger_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
...
class SafewayParser(BaseStoreParser):
    ...
    def _should_skip_safeway_line(self, line: str) -> bool:
        ...
        skip_patterns = [
            r'subtotal', r'tax', r'total', r'savings', r'club card',
            r'discount', r'coupon', r'safeway', r'vons', r'pavilions'
        ]
    ...
    def _parse_safeway_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
...
class CostcoParser(BaseStoreParser):
    ...
    def _should_skip_costco_line(self, line: str) -> bool:
        ...
        skip_patterns = [
            r'subtotal', r'tax', r'total', r'membership', r'executive',
            r'discount', r'coupon', r'costco', r'warehouse'
        ]
    ...
    def _parse_costco_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
...
class WholeFoodsParser(BaseStoreParser):
    ...
    def _should_skip_wholefoods_line(self, line: str) -> bool:
        ...
        skip_patterns = [
            r'subtotal', r'tax', r'total', r'prime', r'discount',
            r'promotion', r'whole foods', r'amazon', r'delivery'
        ]
    ...
    def _parse_wholefoods_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
...
class GenericParser(BaseStoreParser):
    ...
    def _should_skip_generic_line(self, line: str) -> bool:
        """Skip non-item lines"""
        skip_patterns = [
            r'^\s*[-=_]{3,}', r'thank you', r'receipt', r'order',
            r'customer', r'cashier', r'store', r'address', r'phone',
            r'subtotal', r'tax', r'total', r'discount', r'promotion'
        ]
    ...
    def _parse_generic_item_line(self, line: str, line_number: int) -> Optional[ParsedItem]:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                item.confidence_score = self._calculate_item_confidence(item) * 0.7  # Lower confidence for generic
                return item
