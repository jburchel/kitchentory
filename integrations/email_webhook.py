...
class EmailWebhookService:
    ...
    def process_webhook(
        self, service: str, payload: Dict, headers: Dict
    ) -> Dict[str, Any]:
    ...
    def _process_receipt_email(self, user: User, email_data: Dict) -> Dict:
        ...
        import_job = ImportJob.objects.create(
            ...
            status=(
                ...
                if parsed_receipt.confidence_score < 0.8
                else ImportStatus.PROCESSING
            ),
            email_data={
            ...
            raw_data={
                ...
                "total_amount": (
                    float(parsed_receipt.total) if parsed_receipt.total else None
                ),
                "purchase_date": (
                    parsed_receipt.purchase_date.isoformat()
                    if parsed_receipt.purchase_date
                    else None
                ),
        ...
        logger.info(
            f"Created import job {import_job.id} with {items_created} items for user {user.email}"
        )
    ...
    def _auto_process_receipt(self, import_job: ImportJob):
        ...
        logger.info(
            f"Auto-created {created_count} inventory items from import {import_job.id}"
        )
