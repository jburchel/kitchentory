...
class EmailReceiptConfig(models.Model):
    ...
    # Email settings
    email_address = models.EmailField(help_text="Dedicated email for receipt forwarding")
    webhook_secret = models.CharField(max_length=100, blank=True)
    
    # Parsing preferences
    auto_approve = models.BooleanField(default=False, help_text="Automatically add items without review")
    confidence_threshold = models.FloatField(default=0.8, help_text="Minimum confidence for auto-approval")
    
    # Store mappings
    store_mappings = models.JSONField(default=dict, help_text="Map email senders to store names")
...
class ParsedReceiptItem(models.Model):
    ...
    import_job = models.ForeignKey(ImportJob, on_delete=models.CASCADE, related_name='parsed_items')
