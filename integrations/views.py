...
from .models import ImportJob, ImportSource, ImportStatus, ParsedReceiptItem, EmailReceiptConfig
...
def process_receipt_email(household, sender, subject, body):
    ...
    try:
        ...
        try:
            ...
            if config.auto_approve and receipt_data.get('confidence_score', 0) >= config.confidence_threshold:
        ...
        logger.info(f"Processed receipt email for household {household.name}: {import_job.id}")
...
def import_detail(request, job_id):
    ...
    return render(request, 'integrations/import_detail.html', {
        'job': job,
...
def approve_import(request, job_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if result['success']:
                messages.success(request, f"Successfully imported {result['created']} items!")
            else:
...
def email_config(request):
    ...
    if request.method == 'POST':
        ...
        config.confidence_threshold = float(request.POST.get('confidence_threshold', 0.8))
    ...
    return render(request, 'integrations/email_config.html', {
        'config': config
    })
...
def import_detail_view(request, job_id):
    ...
    if import_job.total_items > 0:
        import_job.success_rate = round((import_job.created_items / import_job.total_items) * 100, 1)
    else:
...
def receipt_review(request, job_id):
    ...
    context = {
        ...
        'can_edit': import_job.status in [ImportStatus.PENDING, ImportStatus.PROCESSING],
    }
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if result['success']:
                ...
                return redirect('integrations:receipt_review', job_id=result['import_job_id'])
            else:
...
def approve_receipt(request, job_id):
    ...
    try:
        ...
        created_count = import_job.raw_data.get('auto_created_items', 0) if import_job.raw_data else 0
        messages.success(
            request,
            f"Receipt approved! Created {created_count} inventory items."
        )
    ...
    try:
        ...
        for item_id in selected_items:
            try:
                ...
                quantity = float(request.POST.get(f'quantity_{item_id}', receipt_item.quantity))
                unit = request.POST.get(f'unit_{item_id}', receipt_item.unit)
                category_name = request.POST.get(f'category_{item_id}', default_category.name)
                location_name = request.POST.get(f'location_{item_id}', default_location.name)
        ...
        messages.success(
            request,
            f"Successfully imported {created_count} items to your inventory!"
        )
