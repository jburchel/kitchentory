...
Thank you!
    ...
    print(f"   Store: {amazon_job.raw_data.get('store_name') if amazon_job.raw_data else 'Unknown'}")
    ...
    total_items = sum(job.parsed_items.count() for job in ImportJob.objects.filter(household=household))
    ...
    print(f"✅ Auto-processing: {'Enabled' if auto_result.get('auto_processed') else 'Manual Review Required'}")
