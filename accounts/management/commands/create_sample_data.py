...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        inventory_items = self.create_inventory_items(
            households, products, storage_locations
        )
    ...
    def create_households_and_users(self, num_users):
        ...
        users_data = [
            {
                "username": "sarah_j",
                "email": "sarah@kitchentory.test",
                "first_name": "Sarah",
                "last_name": "Johnson",
            },
            {
                "username": "mike_j",
                "email": "mike@kitchentory.test",
                "first_name": "Mike",
                "last_name": "Johnson",
            },
            {
                "username": "emma_m",
                "email": "emma@kitchentory.test",
                "first_name": "Emma",
                "last_name": "Miller",
            },
            {
                "username": "david_m",
                "email": "david@kitchentory.test",
                "first_name": "David",
                "last_name": "Miller",
            },
            {
                "username": "lisa_s",
                "email": "lisa@kitchentory.test",
                "first_name": "Lisa",
                "last_name": "Smith",
            },
        ]
        ...
        for i, user_data in enumerate(users_data[:num_users]):
            user = User.objects.create_user(password="kitchentory123", **user_data)
    ...
    def create_inventory_items(self, households, products, storage_locations):
        ...
        for household in households:
            ...
            household_users = [
                user for user in User.objects.filter(household=household)
            ]
            if not household_users:
                continue

            household_locations = [
                loc for loc in storage_locations if loc.household == household
            ]
            ...
            for product in selected_products:
                ...
                if product.category and product.category.name in [
                    "Dairy",
                    "Meat & Seafood",
                ]:
                    location = next(
                        (
                            loc
                            for loc in household_locations
                            if loc.name == "Refrigerator"
                        ),
                        household_locations[0],
                    )
                elif product.category and product.category.name == "Frozen":
                    location = next(
                        (loc for loc in household_locations if loc.name == "Freezer"),
                        household_locations[0],
                    )
                    ...
                    location = next(
                        (
                            loc
                            for loc in household_locations
                            if loc.name == "Fruit Bowl"
                        ),
                        household_locations[0],
                    )
                else:
                    location = next(
                        (loc for loc in household_locations if loc.name == "Pantry"),
                        household_locations[0],
                    )
    ...
    def create_recipes(self, products, users):
        ...
        for recipe_data in recipes_data:
            ...
            # Add ingredients
            for order, (ingredient_name, amount, unit) in enumerate(
                recipe_data["ingredients"], 1
            ):
            ...
            for i, step in enumerate(recipe_data["steps"], 1):
                RecipeStep.objects.create(
                    recipe=recipe, step_number=i, instruction=step
                )
