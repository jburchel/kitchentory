...
class ConsumptionPattern(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="consumption_patterns"
    )
    ...
    consumption_unit = models.CharField(
        _("consumption unit"), max_length=10, default="count"
    )
    ...
    average_days_between_purchases = models.IntegerField(
        _("average days between purchases"), default=30
    )
    ...
    next_reorder_prediction = models.DateField(
        _("next reorder prediction"), null=True, blank=True
    )
    ...
    def is_reliable(self):
        ...
        return (
            self.data_points_count >= 3
            and self.confidence_level in ["medium", "high"]
            and self.prediction_accuracy_score >= Decimal("0.6")
        )
...
class UsageEvent(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="usage_events"
    )
    product = models.ForeignKey(
        "inventory.Product", on_delete=models.CASCADE, related_name="usage_events"
    )
    ...
    quantity_used = models.DecimalField(
        _("quantity used"), max_digits=10, decimal_places=3
    )
...
class BudgetTracking(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="budget_trackings"
    )

    # Period definition
    period_type = models.CharField(
        _("period type"), max_length=10, choices=PERIOD_TYPES
    )
    ...
    budget_amount = models.DecimalField(
        _("budget amount"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    actual_spent = models.DecimalField(
        _("actual spent"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )

    # Category breakdown
    produce_budget = models.DecimalField(
        _("produce budget"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    produce_spent = models.DecimalField(
        _("produce spent"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )

    dairy_budget = models.DecimalField(
        _("dairy budget"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    dairy_spent = models.DecimalField(
        _("dairy spent"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )

    meat_budget = models.DecimalField(
        _("meat budget"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    meat_spent = models.DecimalField(
        _("meat spent"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )

    pantry_budget = models.DecimalField(
        _("pantry budget"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    pantry_spent = models.DecimalField(
        _("pantry spent"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )

    other_budget = models.DecimalField(
        _("other budget"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    other_spent = models.DecimalField(
        _("other spent"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    ...
    def calculate_variance(self):
        ...
        if self.budget_amount > 0:
            self.budget_utilization_percentage = (
                self.actual_spent / self.budget_amount
            ) * 100
        else:
...
class ReorderPrediction(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="reorder_predictions"
    )
    product = models.ForeignKey(
        "inventory.Product",
        on_delete=models.CASCADE,
    ...
    consumption_pattern = models.ForeignKey(
        ConsumptionPattern, on_delete=models.CASCADE, related_name="reorder_predictions"
    )

    # Prediction details
    predicted_reorder_date = models.DateField(_("predicted reorder date"))
    predicted_quantity = models.DecimalField(
        _("predicted quantity"), max_digits=10, decimal_places=3
    )
    unit = models.CharField(_("unit"), max_length=10)
    ...
    actual_reorder_date = models.DateField(
        _("actual reorder date"), null=True, blank=True
    )
    actual_quantity = models.DecimalField(
        _("actual quantity"), max_digits=10, decimal_places=3, null=True, blank=True
    )
    ...
    status = models.CharField(
        _("status"), max_length=10, choices=PREDICTION_STATUS, default="pending"
    )
    ...
    def fulfill(self, actual_date, actual_quantity):
        ...
        self.consumption_pattern.prediction_accuracy_score = (
            self._update_pattern_accuracy()
        )
    ...
    def _calculate_date_accuracy(self):
        ...
        if days_diff == 0:
            ...
            return max(
                Decimal("0.00"),
                Decimal("1.00") - (Decimal(str(days_diff)) * Decimal("0.1")),
            )
        else:
            return Decimal("0.00")
    ...
    def _calculate_quantity_accuracy(self):
        ...
        ratio = min(self.actual_quantity, self.predicted_quantity) / max(
            self.actual_quantity, self.predicted_quantity
        )
        return ratio
    ...
    def _update_pattern_accuracy(self):
        ...
        if recent_predictions:
            avg_accuracy = sum(p.accuracy_score for p in recent_predictions) / len(
                recent_predictions
            )
            return avg_accuracy
