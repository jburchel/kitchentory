...
def _calculate_seasonal_multipliers(pattern, usage_events):
    ...
    overall_avg = (
        sum(sum(season_data) for season_data in seasonal_consumption.values())
        / sum(len(season_data) for season_data in seasonal_consumption.values())
        if any(seasonal_consumption.values())
        else 1
    )
    ...
    if recent_additions.exists():
        ...
        if recent_additions.count() > 1:
            ...
            if purchase_intervals:
                pattern.average_days_between_purchases = int(
                    statistics.median(purchase_intervals)
                )
...
def _predict_next_reorder_date(pattern) -> Optional[date]:
    ...
    current_inventory = InventoryItem.objects.filter(
        user=pattern.user, product=pattern.product, is_consumed=False
    ...
    seasonal_multiplier = getattr(
        pattern, f"{current_season}_multiplier", Decimal("1.00")
    )
...
def update_budget_tracking(user, period_type: str = "monthly") -> BudgetTracking:
    ...
    budget_tracking.actual_spent = inventory_spending.aggregate(total=Sum("price"))[
        "total"
    ] or Decimal("0.00")
...
def _calculate_category_spending(budget_tracking, inventory_spending):
    ...
    category_spending = inventory_spending.values("product__category__name").annotate(
        total=Sum("price")
    )
    ...
    for spending in category_spending:
        ...
        setattr(
            budget_tracking,
            field_name,
            current_value + (spending["total"] or Decimal("0.00")),
        )
...
def get_usage_analytics(user, days: int = 30) -> Dict:
    ...
    # Get usage events
    usage_events = UsageEvent.objects.filter(
        user=user, event_date__gte=cutoff_date
    ).select_related("product")

    # Most used products
    most_used = (
        usage_events.values("product__name")
        .annotate(total_usage=Sum("quantity_used"), usage_count=Count("id"))
        .order_by("-total_usage")[:10]
    )

    # Usage by category
    usage_by_category = (
        usage_events.values("product__category__name")
        .annotate(total_usage=Sum("quantity_used"), usage_count=Count("id"))
    ...
    for week in range(4):  # Last 4 weeks
        ...
        week_usage = usage_events.filter(
            event_date__gte=week_start, event_date__lt=week_end
        )
    ...
    return {
        ...
        "cooking_frequency": {
            ...
            "avg_cooking_per_week": (
                cooking_events.count() / 4 if cooking_events.count() > 0 else 0
            ),
        },
